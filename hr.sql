...
IN ( SELECT E.DEPARTMENT_ID, D.SALARY
		MAX(E.SALARY)
	FROM
		EMPLOYEES e
		WHERE
...
FROM
	EMPLOYEES e
JOIN (SELECT DISTINCT E.EMPLOYEE_ID
	FROM
		EMPLOYEES E
	WHERE
		E.SALARY > (
...
FROM EMPLOYEES e WHERE E.LAST_NAME = 'Davies');
...
FROM EMPLOYEES e WHERE (E.SALARY, NVL(E.COMMISSION_PCT,0 ))
IN(
SELECT DISTINCT E.SALARY, NVL(E.COMMISSION_PCT, 0) FROM EMPLOYEES e JOIN DEPARTMENTS D
